<div class="container mx-auto py-12 px-6 text-center">
    ...
    <div class="mt-8 grid gap-6"
        :class="{'grid-cols-1 md:grid-cols-2 lg:grid-cols-3': {{ count($certificates) }} > 1, 'grid-cols-1 max-w-lg mx-auto': {{ count($certificates) }} == 1}">

        @foreach ($certificates as $certificate)
            <div class="p-6 bg-[var(--color-bg-light)] dark:bg-[var(--color-bg-dark)] border border-[var(--color-accent-content)] rounded-lg shadow-lg transition-transform hover:scale-105">
                <!-- Certificate Image -->
